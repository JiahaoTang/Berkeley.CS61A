...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError('The name you input is not exist.')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        if self.operator.string in env:
            operands = []
            for operand in self.operands:
                if type(operand) == Name:
                    operands.append(Number(env[operand.string].value))
                elif type(operand) == CallExpr:
                    operands.append(operand.eval(env))
                else:
                    operands.append(Number(operand.value))
            return env[self.operator.string].apply(operands)
        else:
            raise TypeError
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        for elem in self.parameters:
            if elem in current_env:
                current_env[elem] = arguments[i]
                i += 1
            else:
                current_env.update({elem:arguments[i]})
                i += 1
        used_arguments = []
        for elem in self.body.operands:
            if isinstance(elem, Literal):
                used_arguments.append(elem.eval(current_env))
            elif isinstance(elem, Name):
                used_arguments.append(current_env[elem.string])
            elif isinstance(elem, LambdaExpr):
                used_arguments.append(LambdaExpr.eval(current_env))
        return current_env[self.body.operator.string].apply(used_arguments)
