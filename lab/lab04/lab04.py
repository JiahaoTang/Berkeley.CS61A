...
def replace_elem(lst, index, elem):
    ...
    "*** YOUR CODE HERE ***"
    return [lst[i] for i in range(index)] + [elem] + [lst[i] for i in range(index + 1, len(lst))]
...
def put_piece(board, max_rows, column, player):
    ...
    if max_rows == 0:
        ...
        'return max_rows - 1, board[:max_rows - 1] + [replace_elem(board[max_rows - 1], column, player)] + board[(max_rows):]'
        return max_rows - 1, replace_elem(board, max_rows - 1, replace_elem(board[max_rows - 1], column, player))
    else:
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    if col < 0 or col >= max_cols:
        return -1, board
    current_row_index = max_rows - 1
    
    while current_row_index >= 0 and board[current_row_index][col] != '-':
        current_row_index -= 1
    return put_piece(board, current_row_index + 1, col, player)
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    "*** YOUR CODE HERE ***"
    check_row = board[row]
    correct_list = num_connect * [player]
    for i in range(max_cols - num_connect + 1):
        check_list = check_row[i:i + num_connect]
        if check_list == correct_list:
            return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for row in range(max_rows):
        check_list += get_piece(board, row, col)
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    "*** YOUR CODE HERE ***"
    return check_win_row(board, max_rows, max_cols, num_connect, row, player) or check_win_column(board, max_rows, max_cols, num_connect, col, player) or check_win_diagonal(board, max_rows, max_cols, num_connect, row, col, player)
