...
def make_to_string(front, mid, back, empty_repr):
    ...
    def make_string(lst):
        if lst == Link.empty:
            ...
            return front + str(lst.first) + mid + make_string(lst.rest) + back
    return make_string
...
def add_trees(t1, t2):
    ...
    if t1.is_leaf() and t2.is_leaf():
        return Tree(t1.root + t2.root)
    elif t1.is_leaf() and not t2.is_leaf():
        return Tree(t1.root + t2.root, t2.branches)
    elif not t1.is_leaf() and t2.is_leaf():
        return Tree(t1.root + t2.root, t1.branches)
    else:
        exist_branches = [add_trees(b1, b2) for b1, b2 in zip(t1.branches, t2.branches)]
        if len(t1.branches) > len(t2.branches):
            extra_branches = t1.branches[len(t2.branches):]
        else:
            extra_branches = t2.branches[len(t1.branches):]
        new_branches = exist_branches + extra_branches
        return Tree(t1.root + t2.root, new_branches)
# Link
class Link:
    """A linked list.
...
new_tree = tree_map(lambda x: 2**x, numbers)