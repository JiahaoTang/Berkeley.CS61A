...
def balanced(m):
    ...
    left = sides(m)[0]
    right = sides(m)[1]
    if length(left) * total_weight(left[1]) != length(right) * total_weight(right[1]):
        return False
    elif not is_weight(left[1]) and not is_weight(right[1]):
        return True and balanced(left[1]) and  balanced(right[1])
    else:
        return True
...
def with_totals(m):
    ...
    if is_weight(m):
        ...
        left = sides(m)[0]
        right = sides(m)[1]
        return tree(total_weight(m), [[length(left), with_totals(end(left))], [length(right), with_totals(end(right))]])
